...
namespace NotificationSamples
{
    ...
    public class GameNotificationsManager : MonoBehaviour
    {
        ...
        {
            /// <summary>
            /// Do not perform any queueing at all. All notifications are scheduled with the operating system immediately.
            /// </summary>
            NoQueue = 0x00,
        ...
        //Localized Strings
        public LocalizedString EnergyRefillTitle;
        public LocalizedString EnergyRefillDescrip;
        public LocalizedString ComebackTitle;
        public LocalizedString ComebackDescrip;
        ...
        private void SendEnergyNotification()
        {
            //Debug.Log("Energy notification sent 1");

            IGameNotification notification = CreateNotification();
            if (notification == null) { return; }

            notification.Title = EnergyRefillTitle.GetLocalizedString();
            notification.Body = EnergyRefillDescrip.GetLocalizedString();
            //notification.Group = channelId : ChannelId;
            notification.DeliveryTime = DateTime.Now.AddSeconds(GameManager.Instance.MergeManager.boardController.energyRefillTotalSeconds);
            ...
            //Debug.Log("Energy notification sent 2");
        }
        
        public void SendComeBackNotification()
        {
            //Debug.Log("Come Back notification sent 1");

            IGameNotification notification = CreateNotification();
            if (notification == null) { return; }

            notification.Title = ComebackTitle.GetLocalizedString();
            notification.Body = ComebackDescrip.GetLocalizedString();
            //notification.Group = channelId : ChannelId;
            notification.DeliveryTime = DateTime.Now.AddHours(23);
            notification.SmallIcon = "icon_1";
            notification.LargeIcon = "icon_1";

            PendingNotification notificationToDisplay = ScheduleNotification(notification);
            notificationToDisplay.Reschedule = false;
            //Debug.Log("Come Back notification sent 2");
        }
        ...
        IEnumerator LoadStrings()
        {
            ...
            var loadingOperation = LocalizationSettings.StringDatabase.GetTableAsync("Notifications");
            yield return loadingOperation;

            if (loadingOperation.Status == AsyncOperationStatus.Succeeded)
            {
                var stringTable = loadingOperation.Result;
                EnergyRefillTitle.GetLocalizedString(stringTable);
                EnergyRefillDescrip.GetLocalizedString(stringTable);
                ComebackTitle.GetLocalizedString(stringTable);
                ComebackDescrip.GetLocalizedString(stringTable);
            }
            else
            {
        ...
        private void Start()
        {
            var channel = new GameNotificationChannel("MainChannelID", "Default Game Channel", "Generic notifications");
            Initialize(channel);
        }
